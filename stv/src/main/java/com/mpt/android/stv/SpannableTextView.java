...
import com.mpt.android.stv.span.CenterImageSpan;
...
public class SpannableTextView extends AppCompatTextView {
    ...
    private void applySpannableTo(final Slice slice, SpannableString finalString, int start, int end) {
        ...
        if (slice.getImageResource() != 0) {
            finalString.setSpan(new CenterImageSpan(context, slice.getImageResource())
                    , start
                    , end
                    , Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        }

        if (null != slice.getImageDrawable()) {
            finalString.setSpan(new CenterImageSpan(slice.getImageDrawable())
                        , start
                        , end
                        , Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }

        if (null != slice.getImageBitmap()) {
            finalString.setSpan(new CenterImageSpan(context, slice.getImageBitmap())
                    , start
                    , end
                    , Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
